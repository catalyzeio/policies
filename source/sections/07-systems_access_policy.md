...
1. Requests for access to WatchTower Platform systems and applications is made formally using the following process:
  1. A WatchTower workforce member initiates the access request by creating an Issue in the WatchTower Quality Management System.
  ...
  4. If the review is approved, the Security Officer or Privacy Officer then marks the Issue as Done, adding any pertinent notes required. The Security Officer or Privacy Officer then grants requested access.
     ...
     * For non-production systems, access grants are accomplished by leveraging the access control mechanisms built into those systems. Account management for non-production systems may be delegated to a WatchTower employee at the discretion of the Security Officer or Privacy Officer .
2. Access is not granted until receipt, review, and approval by the WatchTower Security Officer or Privacy Officer ;
...
4. All access to WatchTower systems and services is reviewed and updated on a bi-annual basis to ensure proper authorizations are in place commensurate with job functions. The process for conducting reviews is outlined below:
   1. The Security Officer initiates the review of user access by creating an Issue in the WatchTower Quality Management System.
   2. The Security Officer is assigned to review levels of access for each WatchTower workforce member.
...
5. Any WatchTower workforce member can request change of access using the process outlined in [§7.2 paragraph 1](#7.2-access-establishment-and-modification).
...
8. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of WatchTower Customers to define, and not WatchTower.
...
11. Generic accounts are not allowed on WatchTower systems.
...
1. Role based access categories for each WatchTower system and application are pre-approved by the Security Officer, or an authorized delegate of the Security Officer.
...
3. All Customer support desk interactions must be verified before WatchTower support personnel will satisfy any request having information security implications.
   * WatchTower's current support desk software, Zendesk, requires users to authenticate before submitting support tickets.
   * Support issues submitted via WatchTower's dashboard require that users authenticate with their WatchTower account before submitting support tickets.
...
All workstations at WatchTower are company owned, and all are laptop Apple products running Mac OSX or Linux.
...
9. All workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by WatchTower Health, Inc. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://policy.WatchTower.com) and have completed this training (https://training.WatchTower.com/). Please contact us if you have problems with this - privacy@WatchTower.com.*
...
4. Wireless networks managed within WatchTower non-production facilities (offices, etc.) are secured with the following configurations:
   * All data in transit over wireless is encrypted using WPA2 encryption;
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by the WatchTower Security Officer.
...
3. The Security Officer will terminate users' access rights immediately upon notification, and will coordinate with the appropriate WatchTower employees to terminate access to any non-production systems managed by those employees.
...
2. Disallowing transfer of ePHI to workstations is enforced through technical measures.
   * All production access to systems is performed through a bastion/jump host accessed through a VPN. Direct access to production systems is disallowed by WatchTower's VPN configuration.
   ...
   * Configuration settings for enforcing these technical controls are managed by WatchTower's configuration management tooling, Chef/Salt.

## 7.14 PaaS Customer Access to Systems

WatchTower grants PaaS customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to WatchTower hosted systems.

In the case of data migration, WatchTower does, on a case by case basis, support customers in importing data. In these cases WatchTower requires that all data is secured and encrypted in transit, such as by using SFTP or SCP for transferring files.

In the case of an investigation, WatchTower will assist customers, at WatchTower's discretion, and law enforcement in forensics.
