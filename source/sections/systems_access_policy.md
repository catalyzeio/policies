...
* Requests for access to Agathos systems and applications related to ePHI are made formally to the VP of Engineering, Privacy Officer, or Security Officer.
* Access is not granted until receipt, review, and approval by the Agathos Security Officer;
* The request for access is retained for future reference.
* All access to Agathos systems and services are reviewed and updated on an bi-annual basis to assure proper authorizations are in place commensurate with job functions. The form used to conduct account review is [here](not implemented).
* Any Agathos workforce member can request change of access using this [form](//not implemented).
...
* In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Agathos Customers to define, and not Agathos.
...
* Generic accounts are not allowed on Agathos systems.
...
* Agathos maintains a minimum necessary approach to access to Customer data. As such, Agathos, including all workforce members, does not readily have access to any ePHI.

## Access Authorization

* Role based access categories for each Agathos system and application are pre-approved by the Security Officer or VP of Engineering.
* Agathos utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
* Access to the Agathos Platform systems and applications is controlled by requiring unique User Login ID’s and passwords for each individual user and developer.
...
* Shared accounts are not allowed within Agathos systems or networks.
...
All workstations at Agathos are company owned except for personal computers approved by the VP of Engineering or the Security Officer. All Agathos-owned workstations are Apple products running Mac operating system.
...
* Solicitation of non-company business, or any use of organization’s information systems/applications for personal gain is prohibited. (*Pre-approved personal devices may still be used for personal business as long as no other agathos security policies are violated.*)
...
* Workstation hard drives will be encrypted using FileVault 2.0. (*Except pre-approved personal devices that do not run OSX. These devices must use an equivalent security method.*)
* All workstations have firewalls enabled to prevent unauthorized access unless explicitly granted.
* All organization-owned workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by Agathos, Inc. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://Agathos.io/policy/) and have completed this training (https://training.Agathos.io/). Please contact us if you have problems with this - privacy@Agathos.io.*
* Pre-approved personal devices will be used by no other party than the owner of the device.

## Wireless Access Use

* Agathos production systems are not accessible directly over wireless channels.
...
* Wireless networks managed within Agathos non-production facilities (offices, etc) are secured with the following configurations:
	* All data in transit over wireless is encrypted using WPA2 encryption;
	* SSIDs are not broadcast;
	* Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Agathos Security Officer.
...
Agathos does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Agathos policies.

## Password Management

* User IDs and passwords are used to control access to Agathos systems and may not be disclosed to anyone for any reason.
...
* On all production systems and application in the Agathos environment, password configurations are set to require that passwords are a minimum of 8 character length, 90 day password expiration, account lockout after 5 invalid attempts, password history of last 4 passwords remembered, and account lockout after 15 minutes of inactivity.
...
* Passwords are deactivated immediately upon an employee’s termination (refer to the termination procedures in this policy).
